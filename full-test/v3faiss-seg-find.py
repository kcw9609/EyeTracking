...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 29 10:41:56 2025
            ...
            for mask, label in object_regions:
                if mask[y, x] == 1:
                    ...
                    if indices[0][0] < len(crop_id_list):
                        ...
                        # 검색된 유사 이미지 불러오기
                        matched_crop_path = f"../test-images/{matched_crop_id}"
                        matched_image = cv2.imread(matched_crop_path)

                        if matched_image is not None:
                            # 크롭된 객체와 검색 결과 이미지를 나란히 보여줌
                            cropped_resized = cv2.resize(cropped_object, (300, 300))
                            matched_resized = cv2.resize(matched_image, (300, 300))
                            combined = np.hstack((cropped_resized, matched_resized))
                            cv2.imshow("🔎 Cropped (좌) vs Matched (우)", combined)
                        else:
                            print("❗ 검색된 이미지를 불러올 수 없습니다:", matched_crop_path)
